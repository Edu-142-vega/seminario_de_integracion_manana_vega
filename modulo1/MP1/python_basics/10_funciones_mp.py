...
def saludar():
    print('Hola desde la seccion de expedientes')
saludar()

print('Funcion con parametros')
def saludarConNombre(nombre):
    print(f'Investigador: {nombre}')
...
presentar('Danna', 26, 'Quito')
presentar(ciudad= 'guayaquil',nombre='Juan', edad=40)

print('Funcion con valor por defecto')
def saludo_Con_Parametros_Por_Defecto(nombre, saludo="Informe",puntuacion="f"):
    print(f'{saludo}, {nombre} {puntuacion}')
saludo_Con_Parametros_Por_Defecto('Danna' , "Informe de caso", "...")
saludo_Con_Parametros_Por_Defecto("Juan", puntuacion="...")
saludo_Con_Parametros_Por_Defecto("Carlos", "Reporte")
...
mostrar_info("evidencias","navaja","huellas", "testigo", "video")
